    ...
    for (i=1; i<p_rk+1; ++i) {
        ...
        for (j=1; j<r_rk+1; ++j) {
            ...
            for (k=i; k<p_rk+1; ++k) {
                ...
                sigma = gel(gel(K_ext, k), 3);
                if (k!=i) {
                    gmael2(cup_matrix, j, (2*p_rk-(i-2))*(i-1)/2+k-(i-1)) = stoi(smodis(gneg(my_Artin_symbol(Labs, Lrel, K, idealred0(K,NIpJ, NULL), p_int, sigma)), p_int));
                }
                else {
                    gmael2(cup_matrix, j, (2*p_rk-(i-2))*(i-1)/2+k-(i-1)) = stoi(smodis(gneg(my_Artin_symbol(Labs, Lrel, K, gel(gel(Ja_vect, j), 2), p_int, sigma)), p_int));
                }
        ...
        for (j=1; j<hnf_r_rk+1; j++) {
            for (i=1; i<p_rk+1; ++i) {
                for (k=i; k<p_rk+1; k++) {
                    if (!gequal0(gel(gel(cup_hnf, j), (2*p_rk-(i-2))*(i-1)/2+k-(i-1)))) {
                        if (i==k) {
                            pari_printf("%c^(%d*%Ps)", letters[i-1], p_int, gmael2(cup_hnf, j, (2*p_rk-(i-2))*(i-1)/2+k-(i-1)));
                        }
                        if (i<k) {
